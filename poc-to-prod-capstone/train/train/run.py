...
def train(dataset_path, train_conf, model_path, add_timestamp):
    ...
    # If add_timestamp is True, add subfolder with name equal to execution timestamp '%Y-%m-%d-%H-%M-%S'
    if add_timestamp:
        timestamp = time.strftime('%Y-%m-%d-%H-%M-%S')
        artefacts_path = os.path.join(model_path, "artefacts", timestamp)
    else:
        artefacts_path = os.path.join(model_path, "artefacts")

    # instanciate a LocalTextCategorizationDataset, use embed method from preprocessing module for preprocess_text param
